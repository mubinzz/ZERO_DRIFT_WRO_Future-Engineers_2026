...
def pixel_to_robot_angle(pixel_x, image_width, camera_hfov_deg, camera_angle_offset_deg=0.0):
    """ছবির একটা pixel x-position কে robot-relative angle এ রূপান্তর
    করে (ডিগ্রি, 0=সামনে, ধনাত্মক=ডানে -- lidar_processor.py এর মতোই
    কনভেনশনে, যাতে camera আর LIDAR-এর angle তুলনা/ফিউজ করা যায়)।

    KMIDS-এর pixelToAngle() থেকে অনুপ্রাণিত -- সরল রৈখিক (linear)
    আনুমানিক সূত্র (pinhole/tan-model না): pixel-কে -0.5..+0.5 এ
    normalize করে camera-র HFOV দিয়ে গুণ করা হয়। wide-angle lens-এর
    একদম কিনারায় সামান্য ভুল হতে পারে (আসল projection tan-based),
    কিন্তু calibrate_camera_hfov.py দিয়ে বাস্তব (pixel, angle) জোড়া
    থেকে fit করা camera_angle_offset_deg (ক্যামেরা মাউন্ট করার সময়
    সামান্য বাঁকা থাকলে সেটা ধরার জন্য) এই ভুল অনেকটাই কমিয়ে দেয়।

    camera_hfov_deg/camera_angle_offset_deg : calibrate_camera_hfov.py
        দিয়ে ক্যালিব্রেট করা মান
    """
    normalized = (pixel_x / (image_width - 1)) - 0.5
    return normalized * camera_hfov_deg + camera_angle_offset_deg
